...
print(c.fetchall())
c.execute("SELECT Students.name, Students.surname FROM Students JOIN Student_courses ON Student_courses.student_id = Students.id JOIN Courses ON Student_courses.course_id = Courses.id WHERE Courses.name = 'python' AND Students.city = 'Spb'")
print(c.fetchall())
conn.commit()
conn.close()

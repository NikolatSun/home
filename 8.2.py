...
query2 = Students.select().join(Student_courses).join(
    Courses).where(Courses.name == 'python')

for query in query2:
    print(query.name, query.surname)
print('\n')

query3 = Students.select().join(Student_courses).join(Courses).where(
    Courses.name == 'python').where(Students.city == 'Spb')
